...
@Component
public class LkaMaCriteriasDaoSpringImpl implements LkaMaCriteriasDao {
    ...
    @Autowired
    public LkaMaCriteriasDaoSpringImpl(DataSource dataSource) {
        this.jdbcTemplate = new JdbcTemplate(dataSource);
    }

    private void setLkaMaCriteriasFields() {
        if (lkaList == null) {
            lkaList = lkaDao.findAll();
        }
    }

    private RowMapper<LkaMaCriterias> lkaCriteriasRowMapper = (resultSet, i) -> {
    ...
    @Override
    public int save(LkaMaCriterias lkaCriterias) {
    ...
    @Override
    public LkaMaCriterias find(int id) {
        setLkaMaCriteriasFields();
        List<LkaMaCriterias> result = jdbcTemplate.query(SQL_FIND_BY_ID, lkaCriteriasRowMapper, id);
        return result.size() > 0 ? result.get(0) : null;
    }

    @Override
    public List<LkaMaCriterias> findAll() {
        setLkaMaCriteriasFields();
        return jdbcTemplate.query(SQL_FIND_ALL, lkaCriteriasRowMapper);
    }

    @Override
    public boolean update(LkaMaCriterias lkaCriterias) {
    ...
    @Override
    public void remove(LkaMaCriterias lkaCriterias) {
