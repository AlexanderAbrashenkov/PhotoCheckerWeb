...
public class LkaExpert {
    ...
    public static Map<Integer,String> getReportTypes() {
        Map<Integer, String> reportTypes = new HashMap<>();
        try {
            Connection connection = DataSourcePhotochecker.getDataSource().getConnection();
            ...
            while (resultSet.next()) {
                reportTypes.put(resultSet.getInt("id"), resultSet.getString("type"));
            }
    ...
    public static boolean saveNewUser(User user, String pass) {
        ...
        try {
            ...
            for (int reportType : user.getReportTypeList()) {
                statement.setInt(1, id);
